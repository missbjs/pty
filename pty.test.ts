...
describe("pty.ts — generic PTY service", () => {
  ...
  // ── Output accumulator memory cap ─────────────────────────────────────

  it("captures full output for long-running processes (accum cap doesn't truncate xterm buffer)", async () => {
    // Produce well over the 2KB outputAccum cap to ensure the cap doesn't
    // affect the xterm buffer or the snapshot
    const inst = ptySpawn({
      command: "bash",
      args: ["-c", 'for i in $(seq 1 300); do printf "padded-line-%03d-with-extra-padding\\n" $i; done; sleep 1'],
      cols: 100,
      rows: 24,
    });
    ...
    const snap = ptySnapshot(inst);
    // First and last lines must both be present despite accum being capped
    expect(snap.fullText).toContain("padded-line-001");
    expect(snap.fullText).toContain("padded-line-300");
    // Generated bytes (~10KB) far exceed the 2KB accum cap
    expect(snap.fullText.length).toBeGreaterThan(5000);
...
describe("WebSocket serve/connect", () => {
  ...
  it("starts a serve instance and responds to HTTP health check", async () => {
    ...
    // Spawn the built bundle (verifies the production artifact); --preserve-symlinks-main
    // is a no-op on real files but keeps Node from realpath'ing the entry script when
    // dist/ happens to be reached via a symlink.
    serverProcess = spawn("node", [
      "--preserve-symlinks-main",
      "dist/pty.js",
      "sleep", "30",
      "--", "--serve", "--port", String(testPort),
    ], {
  ...
  it("accepts POST /send with valid body", async () => {
    const res = await fetch(`http://127.0.0.1:${testPort}/send`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: "echo HTTP_SEND_TEST\r" }),
    });
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it("rejects POST /send with missing text", async () => {
    const res = await fetch(`http://127.0.0.1:${testPort}/send`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
  ...
  it("accepts POST /resize and reflects new dimensions in /status", async () => {
    const res = await fetch(`http://127.0.0.1:${testPort}/resize`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("rejects POST /resize with missing dimensions", async () => {
    const res = await fetch(`http://127.0.0.1:${testPort}/resize`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  it("returns 404 for unknown routes", async () => {
    const res = await fetch(`http://127.0.0.1:${testPort}/does-not-exist`);
